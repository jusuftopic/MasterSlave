...
public class Slave implements Runnable {
    ...
    private void listenToMasterMessages() {
        byte[] data = new byte[Message.getMessageSize()];
        try {
            DataInputStream is = new DataInputStream(connectionToPeers.get(connectionToPeers.size() - 1).getInputStream());
            int bread = is.read(data);
            Message msg = new Message(data);
            debugMessage(" got message with from peer with id " + msg.getPeerId() + " and type " + msg.getType());

            handleMessage(connectedPeers.get(connectionToPeers.size() - 1), msg);
        ...
        switch (msg.getType()) {
            case Message.INITIALIZE:
                try {
                    ...
                    os.write((new Message(Message.EXERCISE, id)).toByteArray());
                    os.flush();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            ...
            case Message.EXERCISE:
                try {
                    os = new DataOutputStream(cl.getOutputStream());
                    os.write((new Message(Message.RESULT, id)).toByteArray());
                    os.flush();
